...
export default class Space {
    constructor(dt=1) {
        ...
        this.damp = .8;
        this.vthresh = 3.78;
        this.colors = ['#193741', '#3e5e63', '#588184', '#7ea996', '#90c5a7'];
    }
    ...
    addParticles(groupId, num, ax=0, ay=.8) {
        ...
        let vy;
        for (let i = 0; i < num; i++) {
            vx = randInt(2 * this.defaultV) - this.defaultV;
            vy = randInt(2 * this.defaultV) - this.defaultV;
    ...
    update(c) {
        for (const [groupId, group] of this.groups) {
            for (const [particleId, p] of group.particles) {
                ...
                if (p.x > innerWidth - p.size) {
                    p.x = innerWidth - p.size;
                    p.vx *= -this.damp;
                ...
                if (p.y > innerHeight - p.size) {
                    p.y = innerHeight - p.size;
                    if (Math.abs(p.vy) < this.vthresh) {
                        p.vy = 0;
                        p.ay = 0;
                    } else {
                        p.vy *= -this.damp;
                    }
                } else if (p.y < 0 + p.size) {
                    p.y = 0 + p.size;
                    p.vy *= -this.damp;
                }
            }
